...
def nano_copysource(mapping, OUTPUT="copied_fastq_reads"):
...
def nano_concat(sample_read_dict, mergeby="Sample", output="Concat_reads", filetype="fastq"):
...
def nano_runProg(sample_read_dict, *args, method="qcat", inputflag="-f", outputflag="-o", outfolder=None, outfolder_suffix="out", forceout = False, stdin = False, xlabel="", suffix_change=False, outfmt=""):
...
def nano_filterBC(sample_read_dict, guide_file, copy = True, sampleCOL="Samples_folder", barcodeCOL = "Barcodes", delim="\t", copyout = "Filtered_demutiplex_barcode", mappingout = "filtered_barcode.mapping"):
    ...
    if copy:
        ...
        os.makedirs(copyout, exist_ok=False)
